...
class StructuralAttentionLayer(nn.Module):
    ...
    def forward(self, graph):
        ...
        #assert edge_index[1].max() < alpha.shape[0], "Invalid index in edge_index[1]"

        coefficients = softmax(alpha, edge_index[1],self.num_nodes) # [num_edges, heads]
        ...
        # output
        out = self.act(scatter(x_j * coefficients[:, :, None], edge_index[1], dim=0, dim_size=x.shape[0],reduce="sum"))
...
class TemporalAttentionLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        position_inputs = torch.arange(0,num_inputs).reshape(1, -1).repeat(inputs.shape[0], 1).long().to(inputs.device)
        ...
        outputs = outputs / (num_inputs ** 0.5)
